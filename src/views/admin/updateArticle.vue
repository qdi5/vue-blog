<template>
  <el-form :model="articleForm" ref="articleForm" :rules="rules" label-width="100px" class="demo-ruleForm currentForm">
    ...
    <el-form-item label="文章标签" prop="selectedTag" v-if="articleForm.existedTags">
      <el-select
    ...
    <el-form-item class="declare-btn">
      <el-button type="primary" @click="declareArticle('articleForm')">修改文章</el-button>
      <span v-if="successed === true">修改文章成功，{{ second.number }}秒后跳转到详情页</span>
      <span v-if="successed === false">修改文章失败</span>
    </el-form-item>
...
export default {
  ...
  created () {
    // 不同点一：查询当前文章详情
    this.$http.get(`/article/detail/${this.articleId}`).then(result => {
      console.log('查询详情数据成功', result.data)
      const articleDetailData = result.data.articleDetailData[0]
      const articleTags = result.data.articleTags
      const { title, author, summery, content, typeid } = articleDetailData
      this.articleDetailData = articleDetailData
      this.articleDetail.tags = articleTags
      this.articleForm.title = title
      this.articleForm.author = author
      this.articleForm.summary = summery
      this.articleForm.content = this.turndownService.turndown(content)
      this.articleForm.selectedTag = articleTags.map(obj => obj.tag_id)
      this.articleForm.selectedType = typeid
    })
    // 查询所有文章标签
  ...
  methods: {
    ...
    getImgUrl (response, file, fileList) {
      console.log('获取图片地址', response, file)
    ...
    // 不同之处；
    setMarkdownContent () {
      if (this.mdEditor.value().trim() === '') {
        this.mdEditor.value(this.articleForm.content)
      }
    },
    // 发表文章
    declareArticle (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const summary = this.articleForm.summary
          // 前端选择和创建的标签
          ...
          // 修改文章接口
          this.$http.put(`/article/${this.articleId}`, {
            title,
            author,
            summary,
            existedTagIds,
            ...
            console.log('修改文章的结果：', result)
            if (result.status === 'ok') {
              this.successed = true
              countDown(this.second, () => {
                this.$router.push({ path: `/article/detail/${this.articleId}` })
              })
            } else {
  ...
  watch: {
    'articleForm.content': 'setMarkdownContent'
  },
  components: {
  }
}
